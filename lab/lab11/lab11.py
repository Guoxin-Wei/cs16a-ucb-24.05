...
def eval_and(expressions):
    ...
    # print(expressions.rest == nil)
    if (expressions==nil):
        return True
    if (expressions.first==0 and type(expressions.first)==int):
        return 1
    if (expressions.first==False):
        return False
    firstElement=calc_eval(expressions.first)
    if(not firstElement or expressions.rest == nil):
        return firstElement
...
def eval_define(expressions):
    ...
    # print()
    bindings[expressions.first]=bindings[expressions.rest.first] if expressions.rest.first in bindings else expressions.rest.first
