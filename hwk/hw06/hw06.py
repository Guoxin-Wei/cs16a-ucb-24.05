...
class VendingMachine:
    ...
    def vend(self):
        if not self.stock :
            ...
            self.stock-=1
            yue=self.balance-self.price
            self.balance=0
            return f'Here is your {self.product} and ${yue} change.'
...
def store_digits(n):
    ...
    # print(head)
    while n:
        all_except_last ,last= n//10 ,n%10
        node=Link(last,head)
        head=node
        n=all_except_last
        # print(head)
...
def two_list(vals, counts):
    ...
    "*** YOUR CODE HERE ***"
    ans=()
    upper_bound=len(vals)
    for i in range(upper_bound-1,-1,-1):
        for j in range(counts[i]):
            node=Link(vals[i],ans)
            ans=node
            # print(ans,i,j,vals[i])
    return ans
