...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            if self.is_container and not place.ant.is_container and self.can_contain(place.ant):
    ...
    def nearest_bee(self):
        ...
        while Place is not None:
            if Place.is_hive:
                ...
                if self.lower_bound<=dis<=self.upper_bound:
                    break 
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        def reflected_attack(amount):
            ...
            self.place.bees=nl_bees
        reflected_attack(amount)
        if self.health<=amount:
            reflected_attack(self.damage)
    ...
    def action(self, gamestate):
        ...
        while position:
            if position.ant is not None:
                Aant=position.ant
                if not Aant.is_double:
                    Aant.is_double=True
                    Aant.damage*=2
                if Aant.is_container and Aant.ant_contained is not None:
                    if not Aant.ant_contained.is_double:
                        Aant.ant_contained.is_double=True
                        Aant.ant_contained.damage*=2
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # return self.place.ant is not None or self.place.ant.blocked_path
